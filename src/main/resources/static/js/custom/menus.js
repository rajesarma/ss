			...
			function buildSubMenuUL(parent, items) 
			{
				items = items.sort(function(a,b) 
				{
				  if (a.orderid < b.orderid)
			    ...
			    $.each(items, function () 
			    {
			        if (this.label) 
			        {
			            var li = $("<li class='dropdown'><a href='"+this.target+"'>"+this.label + "</a><div class='dropdown-content'></div></li>");
			            li.appendTo(parent);
			            // if there are sub items, call the buildUL function.
			            if (this.items && this.items.length > 0) 
			            {
